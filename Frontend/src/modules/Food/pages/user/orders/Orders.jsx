    ...
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] pb-24 font-sans">
      ...
      <div className="bg-white dark:bg-[#121212] px-4 pt-3 flex gap-6 border-b dark:border-gray-800 sticky top-[60px] z-10 shadow-sm">
        <button
          ...
          className={`pb-3 text-[15px] font-bold transition-colors relative ${activeTab === 'today' ? 'text-amber-500' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-300'}`}
        >
          Today's Orders
          {activeTab === 'today' && (
            <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-amber-500 rounded-t-md" />
          )}
        </button>
        <button
          onClick={() => setActiveTab('past')}
          className={`pb-3 text-[15px] font-bold transition-colors relative ${activeTab === 'past' ? 'text-amber-500' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-300'}`}
        >
          Order History
          {activeTab === 'past' && (
            <div className="absolute bottom-0 left-0 right-0 h-[3px] bg-amber-500 rounded-t-md" />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {order.restaurantId && (
                        <Link to={`/user/restaurants/${order.restaurantId}`}>
                          <button className="text-xs text-amber-500 font-bold flex items-center mt-1 hover:text-amber-600">
                ...
                <div className="px-4 py-2 flex items-center justify-between">
                  ...
                  <div className="flex items-center ml-4">
                    <Link to={(isDelivered || isCancelled) ? `/user/orders/${order.id}/details` : `/user/orders/${order.id}`}>
                      <button className="text-xs text-amber-500 font-bold hover:text-amber-600 flex items-center gap-1">
                        View Details
      ...
      <div className="flex justify-center mt-8 mb-4">
        <h1 className="text-4xl font-black text-gray-200 dark:text-gray-800 tracking-tighter italic uppercase">tiffinjibites</h1>
      </div>
