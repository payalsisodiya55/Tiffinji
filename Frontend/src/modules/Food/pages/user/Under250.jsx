                ...
                {isClosed ? (
                  ...
                  <Link to="/user/cart" onClick={(e) => e.stopPropagation()}>
                    <Button
                      variant={"outline"}
                      size="sm"
                      className="rounded-xl h-8 sm:h-9 px-4 sm:px-5 text-[12px] sm:text-[14px] font-bold uppercase tracking-wide transition-all duration-300 active:scale-95 flex items-center gap-1 border-amber-500 text-amber-500 hover:bg-amber-500/10"
                    >
                      VIEW CART
                    </Button>
                  </Link>
                  ...
                  <Button
                    variant={"outline"}
                    size="sm"
                    className="rounded-xl h-8 sm:h-9 md:h-10 px-6 sm:px-8 text-[14px] sm:text-[15px] font-bold uppercase transition-all duration-300 active:scale-95 flex items-center justify-center bg-white dark:bg-black border-amber-500 text-amber-500 hover:bg-amber-500/10 shadow-sm"
                    onClick={(e) => {
                      e.stopPropagation()
              ...
              categories.map((category, index) => {
                ...
                return (
                  <div key={category.id} className="flex-shrink-0 cursor-pointer" onClick={() => handleCategorySwitch(isActive ? null : category.id)}>
                      <motion.div
                        className={`flex flex-col items-center justify-center gap-1 w-[76px] h-[76px] sm:w-[88px] sm:h-[88px] md:w-[96px] md:h-[96px] rounded-[1rem] transition-all border ${isActive ? 'bg-amber-500 border-amber-500 text-white shadow-md' : 'bg-white border-gray-100 text-gray-600 dark:bg-[#1a1a1a] dark:border-gray-800 dark:text-gray-300'}`}
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                      >
              ...
              <section key={restaurant.id} className={`p-4 md:p-5 lg:p-6 mb-6 md:mb-8 rounded-2xl bg-slate-50 dark:bg-[#1a1a1a] ${isClosed ? 'opacity-70 grayscale' : ''}`}>
                ...
                <div className="flex items-start justify-between mb-4 md:mb-5">
                  ...
                  <Link to={`/user/restaurants/${restaurantSlug}?under250=true`} className="flex-shrink-0 mt-1">
                    <span className="text-amber-500 font-bold text-sm md:text-base flex items-center hover:underline">
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-4 md:px-6 py-4 md:py-6">
                <div className="flex flex-col gap-3 md:gap-4">
                  {sortOptions.map((option) => (
                    ...
                    >
                      <span className={`text-sm md:text-base lg:text-lg font-medium ${draftSelectedSort === option.id ? 'text-amber-600 dark:text-amber-400 font-bold' : 'text-gray-700 dark:text-gray-300'}`}>
                        {option.label}
                      </span>
                    </button>
                  ))}
              ...
              <div className="flex items-center gap-4 md:gap-6 px-4 md:px-6 py-4 md:py-5 border-t dark:border-gray-800 bg-white dark:bg-[#1a1a1a]">
                ...
                <button
                  onClick={handleApply}
                  className="flex-1 py-3 md:py-4 font-bold rounded-xl transition-colors text-sm md:text-base bg-amber-500 hover:bg-amber-600 text-white shadow-md"
                >
                  Apply
                </button>
              </div>
                ...
                <div className="flex items-center gap-4 md:gap-5 lg:gap-6">
                  ...
                  <Button
                    className={`flex-1 h-[44px] md:h-[50px] lg:h-[56px] rounded-lg md:rounded-xl font-semibold flex items-center justify-center gap-2 text-sm md:text-base lg:text-lg ${shouldShowGrayscale
                      ...
                      : 'bg-amber-500 hover:bg-amber-600 dark:bg-amber-500 dark:hover:bg-amber-600 text-white font-bold shadow-md shadow-amber-500/20'
                      }`}
                    onClick={(e) => {
                      if (!shouldShowGrayscale) {
