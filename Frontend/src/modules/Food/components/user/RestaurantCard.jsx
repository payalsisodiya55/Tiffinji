...
const RestaurantImageCarousel = React.memo(({ restaurant, priority = false, backendOrigin = "" }) => {
  ...
  const images = React.useMemo(() => {
    ...
    } else if (restaurant.profileImage || restaurant.image) {
      sourceImages = [restaurant.profileImage || restaurant.image];
    }
  ...
  return (
    ...
    >
      <OptimizedImage
        ...
        priority={priority}
        className={`w-full h-full object-cover transform scale-100 group-hover:scale-110 transition-transform duration-700 ${loadedBySrc[renderSrc] ? 'opacity-100' : 'opacity-0'
          }`}
        onLoad={() => {
  ...
  return (
    ...
    >
      <div className="relative">
        ...
        >
          <Heart
            className={`w-4 h-4 transition-colors duration-300 ${isFavorite ? "fill-red-500 text-red-500 border-none" : "text-gray-400 stroke-[2.5]"
              }`}
          />
        </button>
      </div>
      ...
      <div className="p-3 sm:p-4">
        ...
        <p className="text-[11px] sm:text-[13px] text-gray-500 mb-2.5 line-clamp-1 font-medium">
          {restaurant.cuisine || "North tiffinji, Chinese"}
        </p>
